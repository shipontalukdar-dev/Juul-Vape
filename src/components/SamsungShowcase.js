...
export default function SamsungShowcase({ theme }) {
  ...
  const colors = {
    ...
    violet: { name: "Titanium Violet", hex: "#5C5380", bg: "bg-indigo-600", border: "border-indigo-400" },
    yellow: { name: "Titanium Yellow", hex: "#EAD075", bg: "bg-amber-400", border: "border-amber-300" },
  ...
  const layers = [
    {
      ...
      component: (
        <div className="w-16 h-20 bg-emerald-500/10 border border-emerald-500/30 rounded-t-lg flex flex-col justify-between p-1.5 backdrop-blur-sm relative">
          ...
          <div className="w-full h-8 bg-emerald-400/20 rounded-sm border border-emerald-400/10 flex items-end relative overflow-hidden">
            <motion.div 
              animate={{ y: [0, 4, 0] }}
              transition={{ duration: 4, repeat: Infinity }}
              className="w-full h-2/3 bg-emerald-400/30"
            />
          </div>
          <span className="text-[7px] text-emerald-400 font-mono tracking-widest text-center">POD_V3</span>
        </div>
      ),
    },
    {
      ...
      component: (
        <div className="w-16 h-10 flex flex-col items-center justify-center relative">
          <div className="w-1 h-8 bg-zinc-400 border border-amber-400/30 rounded-full flex flex-col justify-around py-1">
            <div className="w-2 h-0.5 bg-amber-400 self-center" />
            <div className="w-2 h-0.5 bg-amber-400 self-center" />
            <div className="w-2 h-0.5 bg-amber-400 self-center" />
          </div>
          <div className="w-8 h-2 bg-amber-400/20 border border-amber-400/40 rounded-full mt-1" />
        </div>
      ),
    },
    {
      ...
      component: (
        <div 
          ...
          style={{ 
            backgroundColor: viewMode === "blueprint" ? "rgba(0,113,227,0.05)" : colors[selectedColor].hex,
            borderColor: viewMode === "blueprint" ? "rgba(6,182,212,0.4)" : "#ffffff30"
          }}
        >
          {viewMode === "blueprint" && (
            <div className="absolute inset-0 border border-dashed border-cyan-400/30 bg-cyan-950/20" />
          )}
          ...
          <div 
            ...
            style={{ backgroundColor: viewMode === "blueprint" ? "#22d3ee" : "#ffffff" }}
          />
        </div>
      ),
    },
        ...
        <div className="w-16 h-12 bg-zinc-900 border border-cyan-500/40 rounded-md flex flex-col justify-around p-1 font-mono text-[6px] relative overflow-hidden">
          ...
          <div className="h-0.5 bg-cyan-950 rounded-full w-full overflow-hidden relative">
            <motion.div 
              ...
              className="absolute h-full w-1/3 bg-cyan-400"
            />
          </div>
    ...
    },
    {
      ...
      component: (
        <div className="w-14 h-24 bg-zinc-950 border border-zinc-800 rounded-lg flex flex-col justify-between p-1.5 font-mono text-[7px] text-zinc-500 relative">
          <div className="absolute inset-x-0 top-0 h-1 bg-amber-500/20 border-b border-amber-500/20" />
          <span className="rotate-90 origin-left translate-x-2 translate-y-3 font-bold tracking-wider text-amber-500">SOLID_STATE</span>
          <span className="text-[6px] text-zinc-600 text-right self-end mt-auto">500MAH</span>
        </div>
      ),
    },
    {
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center relative z-10 w-full">
        ...
        <div className="lg:col-span-6 space-y-8 text-center lg:text-left">
          <div className="space-y-4">
            ...
            >
              <Cpu className="w-3.5 h-3.5 text-blue-500" /> SAMSUNG LABS // SMART_AEROSOL
            </motion.div>
                  ...
                  <AnimatePresence>
                    {isExploded && (
                      ...
                      >
                        ...
                        <div className={`text-[8px] font-mono font-bold border-t pt-2 ${isLight ? "border-zinc-100 text-blue-600" : "border-white/5 text-cyan-400"}`}>
                          {layer.specs}
                        </div>
                      </motion.div>
                    )}
